...
const ChatContainer = () => {
  const { message, selectedUser, setSelectedUser, getMessage, sendMessage } = useContext(ChatContext);
  const { authUser, onlineUser } = useContext(AuthContext);

  const scrollEnd = useRef();
  const [input, setinput] = useState('');

  // ✅ handle sending message (fixed recursion issue)
  const handleSendmessage = async (e) => {
    e.preventDefault();
    if (input.trim() === '') return null;
    await sendMessage({ text: input.trim() }); // fixed recursive call
    setinput('');
  };

  // ✅ handle sending an image (use sendMessage instead of handleSendmessage)
